...
local function cs2str (c)
  return m.emulatestreammatch(m.Cs((c + m.P(1)/"")^0), allchar)
end
...
assert(m.emulatestreammatch(m.P(false) + "a", "a") == 2)
assert(m.emulatestreammatch(m.P(true) + "a", "a") == 1)
assert(m.emulatestreammatch("a" + m.P(false), "b") == nil)
assert(m.emulatestreammatch("a" + m.P(true), "b") == 1)

assert(m.emulatestreammatch(m.P(false) * "a", "a") == nil)
assert(m.emulatestreammatch(m.P(true) * "a", "a") == 2)
assert(m.emulatestreammatch("a" * m.P(false), "a") == nil)
assert(m.emulatestreammatch("a" * m.P(true), "a") == 2)
...
assert(m.emulatestreammatch(3, "aaaa"))
assert(m.emulatestreammatch(4, "aaaa"))
assert(not m.emulatestreammatch(5, "aaaa"))
assert(m.emulatestreammatch(-3, "aa"))
assert(not m.emulatestreammatch(-3, "aaa"))
assert(not m.emulatestreammatch(-3, "aaaa"))
assert(not m.emulatestreammatch(-4, "aaaa"))
assert(m.P(-5):emulatestreammatch"aaaa")

assert(m.emulatestreammatch("a", "alo") == 2)
assert(m.emulatestreammatch("al", "alo") == 3)
assert(not m.emulatestreammatch("alu", "alo"))
assert(m.emulatestreammatch(true, "") == 1)
...
assert((word^0 * -1):emulatestreammatch"alo alo")
assert(m.emulatestreammatch(word^1 * -1, "alo alo"))
assert(m.emulatestreammatch(word^2 * -1, "alo alo"))
assert(not m.emulatestreammatch(word^3 * -1, "alo alo"))

assert(not m.emulatestreammatch(word^-1 * -1, "alo alo"))
assert(m.emulatestreammatch(word^-2 * -1, "alo alo"))
assert(m.emulatestreammatch(word^-3 * -1, "alo alo"))

local eos = m.P(-1)

assert(m.emulatestreammatch(digit^0 * letter * digit * eos, "1298a1"))
assert(not m.emulatestreammatch(digit^0 * letter * eos, "1257a1"))

b = {
  [1] = "(" * (((1 - m.S"()") + #m.P"(" * m.V(1))^0) * ")"
}

assert(m.emulatestreammatch(b, "(al())()"))
assert(not m.emulatestreammatch(b * eos, "(al())()"))
assert(m.emulatestreammatch(b * eos, "((al())()(é))"))
assert(not m.emulatestreammatch(b, "(al()()"))

assert(not m.emulatestreammatch(letter^1 - "for", "foreach"))
assert(m.emulatestreammatch(letter^1 - ("for" * eos), "foreach"))
assert(not m.emulatestreammatch(letter^1 - ("for" * eos), "for"))
...
assert(m.emulatestreammatch(caplookfor(letter^1), "   4achou123...") == "achou")
a = {m.emulatestreammatch(caplookfor(letter^1)^0, " two words, one more  ")}
checkeq(a, {"two", "words", "one", "more"})

assert(m.emulatestreammatch( basiclookfor((#m.P(b) * 1) * m.Cp()), "  (  (a)") == 7)
...
a = {m.emulatestreammatch(m.Cc(10,20,30) * 'a' * m.Cp(), 'aaa')}
checkeq(a, {10,20,30,2})
a = {m.emulatestreammatch(m.Cp() * m.Cc(10,20,30) * 'a' * m.Cp(), 'aaa')}
checkeq(a, {1,10,20,30,2})
a = m.emulatestreammatch(m.Ct(m.Cp() * m.Cc(10,20,30) * 'a' * m.Cp()), 'aaa')
checkeq(a, {1,10,20,30,2})
a = m.emulatestreammatch(m.Ct(m.Cp() * m.Cc(7,8) * m.Cc(10,20,30) * 'a' * m.Cp()), 'aaa')
checkeq(a, {1,7,8,10,20,30,2})
a = {m.emulatestreammatch(m.Cc() * m.Cc() * m.Cc(1) * m.Cc(2,3,4) * m.Cc() * 'a', 'aaa')}
checkeq(a, {1,2,3,4})

a = {m.emulatestreammatch(m.Cp() * letter^1 * m.Cp(), "abcd")}
checkeq(a, {1, 5})
...
for i = 250,260 do
  assert(#m.emulatestreammatch(m.C(i), string.rep('a', i)) == i)
...
for n = 1, 550 do
  ...
  assert(m.P(n):emulatestreammatch(x) == n + 1)
  assert(n < 4 or m.emulatestreammatch(m.P(n) + "xxx", x_1) == 4)
  assert(m.C(n):emulatestreammatch(x) == x)
  assert(m.C(m.C(n)):emulatestreammatch(x) == x)
  assert(m.P(-n):emulatestreammatch(x_1) == 1)
  assert(not m.P(-n):emulatestreammatch(x))
  assert(n < 13 or m.emulatestreammatch(m.Cc(20) * ((n - 13) * m.P(10)) * 3, x) == 20)
  local n3 = math.floor(n/3)
  assert(m.emulatestreammatch(n3 * m.Cp() * n3 * n3, x) == n3 + 1)
end

-- true values
assert(m.P(0):emulatestreammatch("x") == 1)
assert(m.P(0):emulatestreammatch("") == 1)
assert(m.C(0):emulatestreammatch("x") == "")

assert(m.emulatestreammatch(m.Cc(0) * m.P(10) + m.Cc(1) * "xuxu", "xuxu") == 1)
assert(m.emulatestreammatch(m.Cc(0) * m.P(10) + m.Cc(1) * "xuxu", "xuxuxuxuxu") == 0)
assert(m.emulatestreammatch(m.C(m.P(2)^1), "abcde") == "abcd")
p = m.Cc(0) * 1 + m.Cc(1) * 2 + m.Cc(2) * 3 + m.Cc(3) * 4


-- test for alternation optimization
assert(m.emulatestreammatch(m.P"a"^1 + "ab" + m.P"x"^0, "ab") == 2)
assert(m.emulatestreammatch((m.P"a"^1 + "ab" + m.P"x"^0 * 1)^0, "ab") == 3)
assert(m.emulatestreammatch(m.P"ab" + "cd" + "" + "cy" + "ak", "98") == 1)
assert(m.emulatestreammatch(m.P"ab" + "cd" + "ax" + "cy", "ax") == 3)
assert(m.emulatestreammatch("a" * m.P"b"^0 * "c"  + "cd" + "ax" + "cy", "ax") == 3)
assert(m.emulatestreammatch((m.P"ab" + "cd" + "ax" + "cy")^0, "ax") == 3)
assert(m.emulatestreammatch(m.P(1) * "x" + m.S"" * "xu" + "ay", "ay") == 3)
assert(m.emulatestreammatch(m.P"abc" + "cde" + "aka", "aka") == 4)
assert(m.emulatestreammatch(m.S"abc" * "x" + "cde" + "aka", "ax") == 3)
assert(m.emulatestreammatch(m.S"abc" * "x" + "cde" + "aka", "aka") == 4)
assert(m.emulatestreammatch(m.S"abc" * "x" + "cde" + "aka", "cde") == 4)
assert(m.emulatestreammatch(m.S"abc" * "x" + "ide" + m.S"ab" * "ka", "aka") == 4)
assert(m.emulatestreammatch("ab" + m.S"abc" * m.P"y"^0 * "x" + "cde" + "aka", "ax") == 3)
assert(m.emulatestreammatch("ab" + m.S"abc" * m.P"y"^0 * "x" + "cde" + "aka", "aka") == 4)
assert(m.emulatestreammatch("ab" + m.S"abc" * m.P"y"^0 * "x" + "cde" + "aka", "cde") == 4)
assert(m.emulatestreammatch("ab" + m.S"abc" * m.P"y"^0 * "x" + "ide" + m.S"ab" * "ka", "aka") == 4)
assert(m.emulatestreammatch("ab" + m.S"abc" * m.P"y"^0 * "x" + "ide" + m.S"ab" * "ka", "ax") == 3)
assert(m.emulatestreammatch(m.P(1) * "x" + "cde" + m.S"ab" * "ka", "aka") == 4)
assert(m.emulatestreammatch(m.P(1) * "x" + "cde" + m.P(1) * "ka", "aka") == 4)
assert(m.emulatestreammatch(m.P(1) * "x" + "cde" + m.P(1) * "ka", "cde") == 4)
assert(m.emulatestreammatch(m.P"eb" + "cd" + m.P"e"^0 + "x", "ee") == 3)
assert(m.emulatestreammatch(m.P"ab" + "cd" + m.P"e"^0 + "x", "abcd") == 3)
assert(m.emulatestreammatch(m.P"ab" + "cd" + m.P"e"^0 + "x", "eeex") == 4)
assert(m.emulatestreammatch(m.P"ab" + "cd" + m.P"e"^0 + "x", "cd") == 3)
assert(m.emulatestreammatch(m.P"ab" + "cd" + m.P"e"^0 + "x", "x") == 1)
assert(m.emulatestreammatch(m.P"ab" + "cd" + m.P"e"^0 + "x" + "", "zee") == 1)
assert(m.emulatestreammatch(m.P"ab" + "cd" + m.P"e"^1 + "x", "abcd") == 3)
assert(m.emulatestreammatch(m.P"ab" + "cd" + m.P"e"^1 + "x", "eeex") == 4)
assert(m.emulatestreammatch(m.P"ab" + "cd" + m.P"e"^1 + "x", "cd") == 3)
assert(m.emulatestreammatch(m.P"ab" + "cd" + m.P"e"^1 + "x", "x") == 2)
assert(m.emulatestreammatch(m.P"ab" + "cd" + m.P"e"^1 + "x" + "", "zee") == 1)
assert(not m.emulatestreammatch(("aa" * m.P"bc"^-1 + "aab") * "e", "aabe"))

assert(m.emulatestreammatch("alo" * (m.P"\n" + -1), "alo") == 4)
...
assert(m.emulatestreammatch(("ab" * #m.P"c")^-1, "abd") == 1)
assert(m.emulatestreammatch(("ab" * m.B"c")^-1, "ab") == 1)
assert(m.emulatestreammatch(("ab" * m.P"cd"^0)^-1, "abcdcdc") == 7)

assert(m.emulatestreammatch(m.P"ab"^-1 - "c", "abcd") == 3)
...
assert(m.emulatestreammatch((m.P(3) +  4 * m.Cp()) * "a", "abca") == 5)
t = {m.emulatestreammatch(((m.P"a" + m.Cp()) * m.P"x")^0, "axxaxx")}
checkeq(t, {3, 6})


-- tests for numbered captures
p = m.C(1)
assert(m.emulatestreammatch(m.C(m.C(p * m.C(2)) * m.C(3)) / 3, "abcdefgh") == "a")
assert(m.emulatestreammatch(m.C(m.C(p * m.C(2)) * m.C(3)) / 1, "abcdefgh") == "abcdef")
assert(m.emulatestreammatch(m.C(m.C(p * m.C(2)) * m.C(3)) / 4, "abcdefgh") == "bc")
assert(m.emulatestreammatch(m.C(m.C(p * m.C(2)) * m.C(3)) / 0, "abcdefgh") == 7)

a, b, c = m.emulatestreammatch(p * (m.C(p * m.C(2)) * m.C(3) / 4) * p, "abcdefgh")
assert(a == "a" and b == "efg" and c == "h")

-- test for table captures
t = m.emulatestreammatch(m.Ct(letter^1), "alo")
checkeq(t, {})

t, n = m.emulatestreammatch(m.Ct(m.C(letter)^1) * m.Cc"t", "alo")
assert(n == "t" and table.concat(t) == "alo")

t = m.emulatestreammatch(m.Ct(m.C(m.C(letter)^1)), "alo")
assert(table.concat(t, ";") == "alo;a;l;o")

t = m.emulatestreammatch(m.Ct(m.C(m.C(letter)^1)), "alo")
assert(table.concat(t, ";") == "alo;a;l;o")

t = m.emulatestreammatch(m.Ct(m.Ct((m.Cp() * letter * m.Cp())^1)), "alo")
assert(table.concat(t[1], ";") == "1;2;2;3;3;4")

t = m.emulatestreammatch(m.Ct(m.C(m.C(1) * 1 * m.C(1))), "alo")
checkeq(t, {"alo", "a", "o"})


-- tests for groups
p = m.Cg(1)   -- no capture
assert(p:emulatestreammatch('x') == 'x')
...
checkerr("rule '1' used outside a grammar", m.emulatestreammatch, m.V(1), "")
checkerr("rule 'hiii' used outside a grammar", m.emulatestreammatch, m.V('hiii'), "")
checkerr("rule 'hiii' undefined in given grammar", m.emulatestreammatch, { m.V('hiii') }, "")
checkerr("undefined in given grammar", m.emulatestreammatch, { m.V{} }, "")
...
assert(m.emulatestreammatch(p, "aaabaac") == 7)

p = m.P'abc' * 2 * -5 * true * 'de'  -- mix of numbers and strings and booleans

assert(p:emulatestreammatch("abc01de") == 8)
...
t = m.emulatestreammatch(m.Ct(m.C('a')^0), string.rep("a", 10000))
...
for _, s in ipairs{" 3 + 5*9 / (1+1) ", "3+4/2", "3+3-3- 9*2+3*9/1-  8"} do
  assert(m.emulatestreammatch(G, s) == loadstring("return "..s)())
end
...
assert(m.emulatestreammatch(g, "abbbcx") == 7)
assert(m.emulatestreammatch(g, "abbbbx") == 2)


-- tests for \0
assert(m.emulatestreammatch(m.R("\0\1")^1, "\0\1\0") == 4)
assert(m.emulatestreammatch(m.S("\0\1ab")^1, "\0\1\0a") == 5)
assert(m.emulatestreammatch(m.P(1)^3, "\0\1\0a") == 5)
assert(not m.emulatestreammatch(-4, "\0\1\0a"))
assert(m.emulatestreammatch("\0\1\0a", "\0\1\0a") == 5)
assert(m.emulatestreammatch("\0\0\0", "\0\0\0") == 4)
assert(not m.emulatestreammatch("\0\0\0", "\0\0"))


-- tests for predicates
assert(not m.emulatestreammatch(-m.P("a") * 2, "alo"))
assert(m.emulatestreammatch(- -m.P("a") * 2, "alo") == 3)
assert(m.emulatestreammatch(#m.P("a") * 2, "alo") == 3)
assert(m.emulatestreammatch(##m.P("a") * 2, "alo") == 3)
assert(not m.emulatestreammatch(##m.P("c") * 2, "alo"))
assert(m.emulatestreammatch(m.Cs((##m.P("a") * 1 + m.P(1)/".")^0), "aloal") == "a..a.")
assert(m.emulatestreammatch(m.Cs((#((#m.P"a")/"") * 1 + m.P(1)/".")^0), "aloal") == "a..a.")
assert(m.emulatestreammatch(m.Cs((- -m.P("a") * 1 + m.P(1)/".")^0), "aloal") == "a..a.")
assert(m.emulatestreammatch(m.Cs((-((-m.P"a")/"") * 1 + m.P(1)/".")^0), "aloal") == "a..a.")

p = -m.P'a' * m.Cc(1) + -m.P'b' * m.Cc(2) + -m.P'c' * m.Cc(3)
assert(p:emulatestreammatch('a') == 2 and p:emulatestreammatch('') == 1 and p:emulatestreammatch('b') == 1)

p = -m.P'a' * m.Cc(10) + #m.P'a' * m.Cc(20)
assert(p:emulatestreammatch('a') == 20 and p:emulatestreammatch('') == 10 and p:emulatestreammatch('b') == 10)



-- look-behind predicate
assert(not m.emulatestreammatch(m.B'a', 'a'))
assert(m.emulatestreammatch(1 * m.B'a', 'a') == 2)
assert(not m.emulatestreammatch(m.B(1), 'a'))
assert(m.emulatestreammatch(1 * m.B(1), 'a') == 2)
...
checkeq(m.emulatestreammatch(x, 'ar cal  c'), {1,3,4,7,9,10})
checkeq(m.emulatestreammatch(x, ' ar cal  '), {2,4,5,8})
checkeq(m.emulatestreammatch(x, '   '), {})
checkeq(m.emulatestreammatch(x, 'aloalo'), {1,7})

assert(m.emulatestreammatch(B, "a") == 1)
assert(m.emulatestreammatch(1 * B, "a") == 2)
assert(not m.B(1 - letter):emulatestreammatch(""))
assert((-m.B(letter)):emulatestreammatch("") == 1)

assert((4 * m.B(letter, 4)):emulatestreammatch("aaaaaaaa") == 5)
assert(not (4 * m.B(#letter * 5)):emulatestreammatch("aaaaaaaa"))
assert((4 * -m.B(#letter * 5)):emulatestreammatch("aaaaaaaa") == 5)

-- look-behind with grammars
assert(m.emulatestreammatch('a' * m.B{'x', x = m.P(3)},  'aaa') == nil)
assert(m.emulatestreammatch('aa' * m.B{'x', x = m.P('aaa')},  'aaaa') == nil)
assert(m.emulatestreammatch('aaa' * m.B{'x', x = m.P('aaa')},  'aaaaa') == 4)



-- bug in 0.9
assert(m.emulatestreammatch(('a' * #m.P'b'), "ab") == 2)
assert(not m.emulatestreammatch(('a' * #m.P'b'), "a"))

assert(not m.emulatestreammatch(#m.S'567', ""))
assert(m.emulatestreammatch(#m.S'567' * 1, "6") == 2)


-- tests for Tail Calls

p = m.P{ 'a' * m.V(1) + '' }
assert(p:emulatestreammatch(string.rep('a', 1000)) == 1001)
...
assert(p:emulatestreammatch(string.rep("00", 10000)))
assert(p:emulatestreammatch(string.rep("01", 10000)))
assert(p:emulatestreammatch(string.rep("011", 10000)))
assert(not p:emulatestreammatch(string.rep("011", 10000) .. "1"))
assert(not p:emulatestreammatch(string.rep("011", 10001)))
...
assert(m.emulatestreammatch("b", "abc", 2))
assert(m.emulatestreammatch("c", "abc", 3))
assert(not m.emulatestreammatch(1, "abc", 4))
assert(m.emulatestreammatch("a", "abc", -3))
assert(m.emulatestreammatch("b", "abc", -2))
assert(m.emulatestreammatch("c", "abc", -1))
assert(m.emulatestreammatch("abc", "abc", -4))   -- truncate to position 1

assert(m.emulatestreammatch("", "abc", 10))   -- empty string is everywhere!
assert(m.emulatestreammatch("", "", 10))
assert(not m.emulatestreammatch(1, "", 1))
assert(not m.emulatestreammatch(1, "", -1))
assert(not m.emulatestreammatch(1, "", 0))
...
s = "hi, this is a test"
assert(m.emulatestreammatch(((p - m.P(-1)) + 2)^0, s) == string.len(s) + 1)
...
s = "hi, this is a test"
assert(m.emulatestreammatch((m.P(1) * p)^0, s) == string.len(s) + 1)
...
p = function (s1, i) return m.emulatestreammatch(m.P"a"^1, s1(1,-1), i) end
...
t = m.emulatestreammatch(m.C(1)^0/f, "abc")
checkeq(t, {"a", "b", "c"})

t = m.emulatestreammatch(m.C(1)^0/f/f, "abc")
checkeq(t, {{"a", "b", "c"}})

t = m.emulatestreammatch(m.P(1)^0/f/f, "abc")   -- no capture
checkeq(t, {{"abc"}})

t = m.emulatestreammatch((m.P(1)^0/f * m.Cp())/f, "abc")
checkeq(t, {{"abc"}, 4})

t = m.emulatestreammatch((m.C(1)^0/f * m.Cp())/f, "abc")
checkeq(t, {{"a", "b", "c"}, 4})

t = m.emulatestreammatch((m.C(1)^0/f * m.Cp())/f, "xbc")
checkeq(t, {4})

t = m.emulatestreammatch(m.C(m.C(1)^0)/f, "abc")
checkeq(t, {"abc", "a", "b", "c"})

g = function (...) return 1, ... end
t = {m.emulatestreammatch(m.C(1)^0/g/g, "abc")}
checkeq(t, {1, 1, "a", "b", "c"})
...
t = m.emulatestreammatch(m.Ct((m.C(1) / function (x,y) return y, x end * m.Cc(1))^0), "abc")
checkeq(t, {nil, "a", 1, nil, "b", 1, nil, "c", 1})

-- tests for Query Replacements

assert(m.emulatestreammatch(m.C(m.C(1)^0)/{abc = 10}, "abc") == 10)
assert(m.emulatestreammatch(m.C(1)^0/{a = 10}, "abc") == 10)
assert(m.emulatestreammatch(m.S("ba")^0/{ab = 40}, "abc") == 40)
t = m.emulatestreammatch(m.Ct((m.S("ba")/{a = 40})^0), "abc")
checkeq(t, {40})

assert(m.emulatestreammatch(m.Cs((m.C(1)/{a=".", d=".."})^0), "abcdde") == ".bc....e")
assert(m.emulatestreammatch(m.Cs((m.C(1)/{f="."})^0), "abcdde") == "abcdde")
assert(m.emulatestreammatch(m.Cs((m.C(1)/{d="."})^0), "abcdde") == "abc..e")
assert(m.emulatestreammatch(m.Cs((m.C(1)/{e="."})^0), "abcdde") == "abcdd.")
assert(m.emulatestreammatch(m.Cs((m.C(1)/{e=".", f="+"})^0), "eefef") == "..+.+")
assert(m.emulatestreammatch(m.Cs((m.C(1))^0), "abcdde") == "abcdde")
assert(m.emulatestreammatch(m.Cs(m.C(m.C(1)^0)), "abcdde") == "abcdde")
assert(m.emulatestreammatch(1 * m.Cs(m.P(1)^0), "abcdde") == "bcdde")
assert(m.emulatestreammatch(m.Cs((m.C('0')/'x' + 1)^0), "abcdde") == "abcdde")
assert(m.emulatestreammatch(m.Cs((m.C('0')/'x' + 1)^0), "0ab0b0") == "xabxbx")
assert(m.emulatestreammatch(m.Cs((m.C('0')/'x' + m.P(1)/{b=3})^0), "b0a0b") == "3xax3")
assert(m.emulatestreammatch(m.P(1)/'%0%0'/{aa = -3} * 'x', 'ax') == -3)
assert(m.emulatestreammatch(m.C(1)/'%0%1'/{aa = 'z'}/{z = -3} * 'x', 'ax') == -3)

assert(m.emulatestreammatch(m.Cs(m.Cc(0) * (m.P(1)/"")), "4321") == "0")

assert(m.emulatestreammatch(m.Cs((m.P(1) / "%0")^0), "abcd") == "abcd")
assert(m.emulatestreammatch(m.Cs((m.P(1) / "%0.%0")^0), "abcd") == "a.ab.bc.cd.d")
assert(m.emulatestreammatch(m.Cs((m.P("a") / "%0.%0" + 1)^0), "abcad") == "a.abca.ad")
...
assert(p:emulatestreammatch("1234567890") == "9 - 1")

assert(m.emulatestreammatch(m.Cc(print), "") == print)

-- too many captures (just ignore extra ones)
p = m.C(1)^0 / "%2-%9-%0-%9"
assert(p:emulatestreammatch"01234567890123456789" == "1-8-01234567890123456789-8")
s = string.rep("12345678901234567890", 20)
assert(m.emulatestreammatch(m.C(1)^0 / "%9-%1-%0-%3", s) == "9-1-" .. s .. "-3")
...
assert(m.emulatestreammatch(m.Cf(m.Cc(0) * m.C(1)^0, f), "alo alo") == 7)
...
local function find (p, s)
  return m.emulatestreammatch(basiclookfor(p), s)
end
...
assert(m.emulatestreammatch({'a' * -m.V(1)}, "aaa") == 2)
assert(m.emulatestreammatch({'a' * -m.V(1)}, "aaaa") == nil)
...
for i = 1, 10 do
  g[1] = "i"..i
  local p = m.P(g)
  assert(p:emulatestreammatch("aaaaaaaaaaa") == 11 - i + 1)
end
...
x = {(m.Cmt(1, id)^0):emulatestreammatch(string.rep('a', 500))}
assert(#x == 500)
...
assert(table.concat{p:emulatestreammatch('abababab')} == string.rep('137', 4))

local function ref (s, i, x)
  return m.emulatestreammatch(x, s(1,-1), i - x:len())
end

assert(m.Cmt(m.P(1)^0, ref):emulatestreammatch('alo') == 4)
assert((m.P(1) * m.Cmt(m.P(1)^0, ref)):emulatestreammatch('alo') == 4)
assert(not (m.P(1) * m.Cmt(m.C(1)^0, ref)):emulatestreammatch('alo'))

ref = function (s,i,x) return i == tonumber(x) and i, 'xuxu' end

assert(m.Cmt(1, ref):emulatestreammatch'2')
assert(not m.Cmt(1, ref):emulatestreammatch'1')
assert(m.Cmt(m.P(1)^0, ref):emulatestreammatch'03')
...
assert(not c:emulatestreammatch'[[]=]====]=]=]==]===[]')
...
do
  ...
  p:emulatestreammatch(string.rep('1', 10))
  assert(c == 11)
end
...
local match, compile = re.emulatestreammatch, re.compile
...
assert(b == m.P(b))
assert(b:emulatestreammatch"((((a))(b)))")
...
assert(e:emulatestreammatch("2.34") == math.sin(2.34))
...
assert(c:emulatestreammatch'[==[]]===]]]]==]===[]' == 17)
assert(c:emulatestreammatch'[[]=]====]=]]]==]===[]' == 14)
assert(not c:emulatestreammatch'[[]=]====]=]=]==]===[]')

c = re.compile" '[' {:init: '='* :} '[' (!(']' =init ']') .)* ']' =init ']' !. "

assert(c:emulatestreammatch'[==[]]===]]]]==]')
assert(c:emulatestreammatch'[[]=]====]=][]==]===[]]')
assert(not c:emulatestreammatch'[[]=]====]=]=]==]===[]')
...
assert(x:emulatestreammatch"alo alo" == "+ +")
...
assert(p:emulatestreammatch'aaabbb')
assert(p:emulatestreammatch'aaa')
assert(not p:emulatestreammatch'bbb')
assert(not p:emulatestreammatch'aaabbba')

-- testing groups
t = {re.emulatestreammatch("abc", "{:S <- {:.:} {S} / '':}")}
checkeq(t, {"a", "bc", "b", "c", "c", ""})

t = re.emulatestreammatch("1234", "{| {:a:.:} {:b:.:} {:c:.{.}:} |}")
checkeq(t, {a="1", b="2", c="4"})
t = re.emulatestreammatch("1234", "{|{:a:.:} {:b:{.}{.}:} {:c:{.}:}|}")
checkeq(t, {a="1", b="2", c="4"})
t = re.emulatestreammatch("12345", "{| {:.:} {:b:{.}{.}:} {:{.}{.}:} |}")
checkeq(t, {"1", b="2", "4", "5"})
t = re.emulatestreammatch("12345", "{| {:.:} {:{:b:{.}{.}:}:} {:{.}{.}:} |}")
checkeq(t, {"1", "23", "4", "5"})
t = re.emulatestreammatch("12345", "{| {:.:} {{:b:{.}{.}:}} {:{.}{.}:} |}")
...
assert(p:emulatestreammatch"add(mul(a,b), apply(f,x))" == "a * b + f(x)")

rev = re.compile[[ R <- (!.) -> '' / ({.} R) -> '%2%1']]

assert(rev:emulatestreammatch"0123456789" == "9876543210")
